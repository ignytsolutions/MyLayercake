...
namespace MyLayercake.Sql.Relex {
	/// <summary>
	...
	public class RelexBuilder {
		...
		public string BuildRelex(Query.Query node) {
			InternalBuilder builder = new InternalBuilder();
		...
		class InternalBuilder : SqlExpressionBuilder {
			...
			public override string BuildExpression(QNode node) {
				if (node is Query.Query)
					return BuildQueryString((Query.Query)node, false);
			...
			public string BuildQueryString(Query.Query q, bool isNested) {
			...
			public override string BuildValue(IQueryValue value) {
				if (value is Query.Query)
					return BuildQueryString((Query.Query)value, true);
				if (value is QRawSql)
